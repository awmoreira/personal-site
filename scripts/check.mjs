...
try {
  ...
  for (const lang of ["en", "pt"]) {
    ...
    const visible = html
      ...
      .replace(/<[^>]+>/g, " ");
    assert(
      !/Lighthouse/i.test(visible),
      "Gavea highlights shipped architecture rather than a benchmark",
    );
    ...
    const pdf = await fetch(`${origin}/resume/allan-winckler-${lang}.pdf`);
    assert.equal(pdf.status, 200);
    assert((await pdf.text()).startsWith("%PDF-"));
    const og = await fetch(`${origin}/og-${lang}.png`);
  ...
  console.log(
    "PASS: default language, static EN/PT content, anchors, metadata, factual regressions, PDFs, OG images and 404s.",
  );
} finally {
  server?.kill("SIGTERM");
}
